...
namespace AIsdk
{
	public class Vimo
	{
		...
		public OcrResponse RunOrderOcr(Mat image)
		{
			...
			{
				toolClass.SaveLog($"OCR识别异常: {ex.Message}");
				return null;
			}
		}

		public OcrResponse RunOrderOcr(Mat image, OpenCvSharp.Rect roi)
		{
			if (module == null) return null;
			...
			{
				OcrResponse results;
				int ret = Run_OrderOcr(image, roi, out results);
				if (ret == 0 && results != null)
				{
					return results;
				}
		...
		/// 运行分割识别
		/// </summary>
		public SegmentationResponse RunSegmentation(Mat image)
		{
			if (module_segmentation == null) return null;

			try
			{
				SegmentationResponse results;
				int ret = Run_Segmentation(image, out results);
				if (ret == 0 && results != null)
				{
					return results;
				}
			...
			{
				toolClass.SaveLog($"分割识别异常: {ex.Message}");
				return null;
			}
		}
		...
		/// 运行分类识别
		/// </summary>
		public ClassificationResponse RunClass(Mat image, Rect roi)
		{
			...
			try
			{
				...
				int ret = Run_Class(image, roi, out results);
				if (ret == 0 && results != null)
				{
					return results;
				}
			...
			{
				toolClass.SaveLog($"分类识别异常: {ex.Message}");
				return null;
			}
		}
